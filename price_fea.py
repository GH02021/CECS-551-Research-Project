...
if not os.path.exists("merged_OHLCV_Sentiment.csv"):
    raise FileNotFoundError("merged_OHLCV_Sentiment.csv not found in the current directory.")

df = pd.read_csv("merged_OHLCV_Sentiment.csv")
...
df.to_csv("data/final/price_features.csv", index=False)
...
plt.savefig("plots/violin_rsi_vs_label.png")
plt.show()
...
if 'label_up_next' in df.columns:
    ...
    # Ensure output directory exists
    os.makedirs("plots", exist_ok=True)
    plt.savefig("plots/rsi_vs_label.png")
    plt.show()
else:
