...
output_dir = "sentiment_curation_output"
os.makedirs(output_dir, exist_ok=True)
print(f"\nSaving features to '{output_dir}' directory...")

# --- 1. Load and Prepare Base Data ---
try:
    df_base = pd.read_csv("Processed Datasets/merged_OHLCV_Sentiment.csv")
...
plt.savefig("sentiment_curation_output/sentiment_zscore_comparison.png")
...
plt.tight_layout()
plt.savefig("sentiment_curation_output/sentiment_vs_price_comparison.png")
...
"""# Report Analysis
To prepare the raw FinBERT pos - neg sentiment scores for our model, we first perform feature engineering steps. We begin by filling the 660 NaN values (days with no news) with a neutral 0.0 and applying a short-term rolling mean utilizing 3-day, 5-day, or 7-day as "hyperparameters." to filter out daily statistical "noise" and capture the more stable, underlying sentiment trend. The reason behind testing these hyperparameters to capture the market's mood (sentiment) changes in different durations. Next, we normalize this smoothed signal using a 20-day rolling z-score, which makes the feature adaptive by re-framing it to answer, "How unusual is today's sentiment compared to the last 20 days?" Finally, we create binary sent_high and sent_low flags that trigger whenever this z-score exceeds the +/- 1.5 threshold. This is a statistical choice to intentionally filter out the "normal" 86.6% of daily fluctuations, providing the model with a clean, explicit signal that only fires on days of statistically significant sentiment shifts. This multi-step process transforms a volatile raw score into a set of stable, normalized, and high-impact features, which dramatically improves the model's prediction consistency.
"""
